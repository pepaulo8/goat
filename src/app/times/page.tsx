...
export default function Times() {
  ...
  const sortearTimes = (rachaData: any, potesData: {pote: number, jogadores: []}[]) => {
    setIsLoading(true);
    ...
    potesData.forEach((pote) => {
      const shuffledPote = [...pote.jogadores].sort(() => Math.random() - 0.5); // Embaralha os jogadores do pote
  ...
  return (
    ...
    <div style={{ maxWidth: '800px', margin: '50px auto' }}>
      ...
      <button onClick={() => 
    sortearTimes(racha, potes)} style={{ marginTop: '20px' }}>
        Sortear novamente
      </button>
      <br></br>
